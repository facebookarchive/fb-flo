...
function Flo(dir, options, callback) {
  ...
  this.dir.forEach(function(folder){
    this.watchers[folder] = new sane(folder, {
      glob: options.glob,
    ...
    this.watchers[folder].once('ready', this.emit.bind(this, 'ready'));
...
Flo.prototype.normalizeResource = normalizeResource;

Flo.prototype.broadcast = function(resource){
    this.server.broadcast(resource);
};
